...
["415079610", "LUK"],
    ...
    # Loop sui prodotti
    for prod_info in PRODOTTI:
        # Estrai codice e brand
        codice_iap = prod_info[0]
        brand = prod_info[1] if len(prod_info) > 1 else ""

        # Cerca il prodotto su IAP
        iap_token = get_iap_token(silent=True)
        resp = get_magazzino_products(iap_token, search_term=codice_iap)
        batches = resp.get("data", [])

        if not batches:
            print(f"⚠️ Nessun batch per {codice_iap}")
            continue

        products = batches[0].get("data", [])
        if not products:
            print(f"⚠️ Nessun prodotto per {codice_iap}")
            continue
        ...
        for p in products:
            ...
            # Costruisci payload base passando SKU e brand custom
            item = ebay_utils.map_basic_item(p, custom_sku=codice_iap, custom_brand=brand)
